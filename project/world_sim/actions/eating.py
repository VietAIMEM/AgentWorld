...
class BuyFoodAction(Action):
    ...
    def can_execute(self, npc, world) -> bool:
        return (
            npc.location_id == world.market_id
            and world.is_shop_open()
            and world.economy.can_buy_food(npc, world)
        )
    ...
    def apply(self, npc, world) -> None:
        if npc.inventory.get("food", 0) >= self.reserve:
            return
        if world.economy.buy_food(npc, world):
